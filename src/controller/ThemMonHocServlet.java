...
/**
 * Servlet implementation class ThemMonHocServlet
 */
public class ThemMonHocServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ThemMonHocServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("submit".equals(request.getParameter("submit"))) {
			...
			try {
				MonHocBO.getInstance().add(new MonHoc(tenMH));
			...
			RequestDispatcher rd = request.getRequestDispatcher("monhoc/them.jsp");
			rd.forward(request, response);
		}
	}
