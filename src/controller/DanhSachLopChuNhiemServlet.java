...
import model.bo.LopChuNhiemBO;
import model.bo.NamHocBO;
import model.bo.PhuHuynhBO;
import model.bo.TaiKhoanBO;

/**
 * Servlet implementation class DanhSachLopChuNhiem
 */
public class DanhSachLopChuNhiemServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DanhSachLopChuNhiemServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ArrayList<LopChuNhiem> lcn = LopChuNhiemBO.getInstance().getAll();
			request.setAttribute("listNH", nh);
			request.setAttribute("listLCN", lcn);
			RequestDispatcher rd = request.getRequestDispatcher("lopchunhiem/danhSach.jsp");
			rd.forward(request, response);
		} catch (SQLException e) {
			e.printStackTrace();
		}
