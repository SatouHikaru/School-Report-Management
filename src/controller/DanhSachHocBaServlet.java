...
/**
 * Servlet implementation class DanhSachHocBaServlet
 */
public class DanhSachHocBaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public DanhSachHocBaServlet() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			ArrayList<HocSinh> hocSinh = HocSinhBO.getInstance().getAll("", 0, 0);

			request.setAttribute("listHS", hocSinh);
			RequestDispatcher rd = request.getRequestDispatcher("/hocba/danhSach.jsp");
			rd.forward(request, response);
		} catch (SQLException e) {
			e.printStackTrace();
		}
