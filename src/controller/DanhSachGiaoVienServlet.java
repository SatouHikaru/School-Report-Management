...
/**
 * Servlet implementation class DanhSachGiaoVienAdminServlet
 */
public class DanhSachGiaoVienServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DanhSachGiaoVienServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ArrayList<GiaoVien> gv = GiaoVienBO.getInstance().getAll(term, maMH);
			request.setAttribute("listMH", mh);
			request.setAttribute("listGV", gv);
			RequestDispatcher rd = request.getRequestDispatcher("/giaovien/danhSach.jsp");
			rd.forward(request, response);
		} catch (SQLException e) {
			e.printStackTrace();
		}
