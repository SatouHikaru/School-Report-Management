...
/**
 * Servlet implementation class SuaHocSinhServlet
 */
public class SuaHocSinhServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SuaHocSinhServlet() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("submit".equals(request.getParameter("submit"))) {
			String tenHS = request.getParameter("hoTen");
			...
			String maPH = request.getParameter("phuHuynh");
			
			try {
				HocSinhBO.getInstance().edit(new HocSinh(maHS, tenHS, Boolean.valueOf(gioiTinh), LocalDate.parse(ngaySinh, formatter), dienThoai, diachi, Long.valueOf(maLop), Long.valueOf(maPH)));
			...
			ArrayList<Lop> lop = null;
			ArrayList<PhuHuynh> ph = null;
			HocSinh hs = null;
			try {
				hs = HocSinhBO.getInstance().getById(maHS);
				lop = LopBO.getInstance().getAll();
				ph = PhuHuynhBO.getInstance().getAll("", 0);
			} catch (SQLException e) {
				e.printStackTrace();
			}
			...
			request.setAttribute("listPH", ph);
			RequestDispatcher rd = request.getRequestDispatcher("hocsinh/sua.jsp");
			rd.forward(request, response);
		}
	}
