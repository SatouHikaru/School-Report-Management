...
/**
 * Servlet implementation class SuaMonHocServlet
 */
public class SuaMonHocServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SuaMonHocServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("submit".equals(request.getParameter("submit"))) {
			...
			try {
				MonHocBO.getInstance().edit(new MonHoc(Long.valueOf(maMH), tenMH));
			} catch (SQLException e) {
				e.printStackTrace();
			}
			...
			RequestDispatcher rd = request.getRequestDispatcher("monhoc/sua.jsp");
			rd.forward(request, response);
		}
	}
